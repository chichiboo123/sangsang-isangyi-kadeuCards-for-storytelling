...
export function downloadTxtFile(content: string): void {
  const blob = new Blob([content], { type: 'text/plain;charset=utf-8' });
  const url = URL.createObjectURL(blob);
  const link = document.createElement('a');
  link.href = url;
  link.download = `story_${new Date().toISOString().split('T')[0]}.txt`;
...
export async function generatePDF(storyText: string, flippedCards: CardData[]): Promise<void> {
  ...
  // Save the PDF
  doc.save(`story_${new Date().toISOString().split('T')[0]}.pdf`);
}
